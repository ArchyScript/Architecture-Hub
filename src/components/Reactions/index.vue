  ...
  >
    ...
    <div class="w-full flex items-center justify-around space-x-2">
      ...
      >
        ...
        <!-- <span>{{ reactions.no_of_likes }}</span> -->
        <!-- <span v-if="no_of_likes < 1">{{ '' }}</span>
        <span v-if="no_of_likes >= 1">{{ no_of_likes }}</span> -->
        <!-- <span>{{ no_of_likes }}</span> -->
      </span>
...
export default {
  ...
  setup(props: any) {
    ...
    const displayPostReactions = async () => {
      ...
      no_of_likes.value = props.reactions.no_of_likes
